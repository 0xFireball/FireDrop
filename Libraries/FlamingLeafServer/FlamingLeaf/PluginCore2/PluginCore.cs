...
namespace Aluminum.PluginCore2
{
	/// <summary>
	/// Summary description for PluginServices.
	/// </summary>
	public class PluginServices : IFlamingPluginHost   //<--- Notice how it inherits IPluginHost interface!
	{
		/// <summary>
		...
		{
			FindPluginsByPath(AppDomain.CurrentDomain.BaseDirectory);
		}
		/// <summary>
		/// Searches the passed Path for Plugins
		/// </summary>
		/// <param name="Path">Directory to search for Plugins in</param>
		public void FindPluginsByPath(string Path)
		{
		...
		public void FindPluginsBySuffix(string Path, string Suffix)
		{		
			//Go through all the files in the plugin directory
			foreach (string fileOn in Directory.GetFiles(Path))
			{
				...
				//Preliminary check, must be .dll
				if (file.Extension.Equals(".dll") && file.Name.EndsWith(Suffix))
				{	
					//Add the 'plugin'
					this.AddPlugin(fileOn);		
				}
	...
	namespace Types
	{
		...
		public class AvailablePlugin
		{
			...
			private IFlamingPlugin myInstance = null;
			private string myAssemblyPath = "";
			
			public IFlamingPlugin Instance
			{
